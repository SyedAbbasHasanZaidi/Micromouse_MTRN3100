...
int task31() {
  ...
  const int obstacleThreshold = 105;
  ...
  if (distance < 0 || distance > obstacleThreshold + tolerance) {
    Forward();
    Serial.println("No object detected — moving FORWARD");
  } else if (distance <= obstacleThreshold + tolerance && distance >= obstacleThreshold - tolerance) {
    Halt();
